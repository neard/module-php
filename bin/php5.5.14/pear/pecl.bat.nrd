...
FOR %%x IN ("~NEARD_WIN_PATH~\bin\php\php5.5.14\pear\pear") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=~NEARD_WIN_PATH~\bin\php\php5.5.14\pear\pear"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=~NEARD_WIN_PATH~\bin\php\php5.5.14\pear"
IF "%PHP_PEAR_PHP_BIN%"=="" SET "PHP_PEAR_PHP_BIN=~NEARD_WIN_PATH~\bin\php\php5.5.14\php.exe"
GOTO :INSTALLED
